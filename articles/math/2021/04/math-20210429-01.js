MultipePlatformBlogData.register(doc => {
    ...
    doc.body(
        ...
        , h4("命題")
        , p(ol(
            li("ある", r, "に対して", tex("a^{p^r}∈K"), "なら、"
                , n, "は", tex("1"), "または", tex("p"), "冪（", tex("<= p^r"), "）。")
            , li(a, "が", K, "上が純非分離的"
                , tex("⇔"), "ある", r, "に対して", tex("a^{p^r}∈K"))
            , li("純非分離的な元の集合は, ", tex("L/K"), "の中間体を成す。")
            , li(tex("L/K"), "が有限次拡大であるとき", br(), tex("L/K"), "が純非分離的"
                , tex("⇔"), L, "は", K, "上純非分離的な元で生成される。")
            , li(
                tex("L/K"), "が有限次純非分離拡大ならば、拡大体の列"
                , p(textCenter, tex("K = K_0 ⊂ K_1 ⊂ … ⊂ K_i ⊂ … ⊂ K_r = L"))
                , p(textCenter, tex("[K_{i+1} : K_i] = p"))
                , "が存在する。よって、拡大次数", tex("[L : K]"), "は", tex("p"), "冪になる。"
            )
            , li(M, "を", tex("L/K"), "の中間体とすると",
                p(textCenter, tex("L/K"), "は純非分離的", tex("⇔ L/M, M/K"), "がそれぞれ純非分離的",))
        ))
        , h4("証明")
        , h5("1.")
        , p("体の列")
